 ...
 *
 *   - platform modules (react, cordis, the dsh-client-* UI layers) stay
 *     external — the loader's module table answers them, and inlining a second
 *     copy would break runtime identity across plugins;
 *   - a purity gate turns any other `@deepseek-ai/*` value import into a build
 *     error rather than a silently duplicated instance;
 *   - CSS Modules are compiled by lightningcss and injected as a `<style>` tag,
 *     because the loader has no stylesheet channel.
 */
 ...
 *
 * tsc emits into `lib/client/`, but `.module.css` files are never emitted —
 * they only exist under `src/client/`. Resolve against the importer first,
 * then retry with the `lib` path segment rebased onto `src`.
 * @param source - the import specifier, relative to the importer.
 * @param importer - the emitted module doing the importing.
 * @returns an absolute path to the stylesheet source.
 */
function sourceAssetPath(source: string, importer: string): string {
  const emitted = resolve(dirname(importer), source)
  if (existsSync(emitted)) return emitted
  return emitted.replace(`${sep}lib${sep}`, `${sep}src${sep}`)
}

const config: UserConfig = {
  name: `${PLUGIN_ID}/client`,
  // The tsc client program's output, not source: type-checking and emit stay
  // with tsc so the two programs keep their separate lib/type environments.
